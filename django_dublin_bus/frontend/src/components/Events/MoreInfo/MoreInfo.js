...
export default function MoreInfo(SelEvent) {
  ...
  try {
    date = SelEvent.dates.start.localDate;
    time = SelEvent.dates.start.localTime;
  } catch (error) {}

  const EventMap = compose(
    withProps({
      googleMapURL:
        "https://maps.googleapis.com/maps/api/js?v=3.exp&libraries=geometry,drawing,places&key=AIzaSyA3pi7A-nqYC6wrN-i_pupw3_UCv8lHqzA",
    ...
    >
      <Marker position={{ lat: venuelat, lng: venuelong }} />
    </GoogleMap>
  ));
    ...
    <InfoContainer>
      <InfoWindow>
        <TextWrapper>
          ...
          <EventDate>
            {new Date(SelEvent.dates.start.dateTime).toLocaleDateString(
              "en-gb",
